...
export default class History {
    ...
    public load(): Promise<HistoryEntry[]> {
        return new Promise(function (resolve, reject) {
            chrome.storage.sync.get(historyKeyOfStorage, function (result: { [id: string]: any }) {
                Logger.log("load: Data in Storage: " + JSON.stringify(result));
    ...
    public async save(historyItemToStore: HistoryEntry): Promise<void> {
        ...
        if (resultArray.filter((historyItem: HistoryEntry) => historyItem.url.includes(historyItemToStore.url)).length <= 0) {
            ...
            Logger.log("History Item Stored for Host: " + historyItemToStore.url);
            ...
            Logger.log("Data to be save to storage (stored): " + JSON.stringify(history));

            return new Promise(function (resolve, reject) {
                chrome.storage.sync.set(history, function () {
                    Logger.log('Saved new history object to Chrome Storage.' + JSON.stringify(history));
                    // store worked, resolve now.
                    chrome.runtime.lastError ? reject(Error(chrome.runtime.lastError.message)) : resolve();
                });
            });
        } else {
            Logger.log("There was already an Entry for: " + historyItemToStore.url + ". No need to update Data in Storage.");
        }
    }

    public async getLastFound(host: string) {
        Logger.log("getLastFound for: " + host);
        // get the data from the storage
        let resultArray = await this.load();
        Logger.log("Data loaded: " + JSON.stringify(resultArray) + ", Len: " + resultArray.length);
        for (let i = 0; i < resultArray.length; i++) {
            Logger.log("Counter: " + i + ", URL: " + resultArray[i].url.includes(host));
            if (resultArray[i].url.includes(host)) {
                Logger.log(JSON.stringify(resultArray[i]));
