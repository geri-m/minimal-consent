...
export default class TrustArc implements ICmp {
    ...
    public async handleCmp() {
        ...
        Utils.log("MC Button: " + minimalConsentButton);
        ...
        if (Utils.objectClickable(divTruste) && this._cmp.state === 0) {
            ...
            window.addEventListener('message', async (event) => {
                ...
                Utils.log("#" + JSON.stringify(eventJson));
                if (eventJson.source === "preference_manager" && eventJson.message === "change_panel") {
                    ...
                    await this.sleep(1000); // Waiting for a second

                    if (Utils.objectClickable(minimalConsentButton)) {
